    ...
    <main style={{ flex: 1, padding: '100px 48px 60px', display: 'flex', justifyContent: 'center' }}>
      <div style={{ maxWidth: 720, width: '100%' }}>
        ...
        <div style={{ marginBottom: 40 }}>
          ...
          {featuredArticle && (
            <div
              onClick={() => onNavigateToArticle(featuredArticle)}
              style={{ background: '#fff', border: '1px solid #e0dbd2', borderRadius: 16, padding: '24px 28px', cursor: 'pointer', transition: 'all 0.2s' }}
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16 }}>
                <div style={{ flex: 1 }}>
                  ...
                  <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: 18, fontWeight: 700, color: '#1a1714', marginBottom: 8, lineHeight: 1.3 }}>{featuredArticle.title}</h3>
                  <p style={{ fontSize: 14, color: '#4a4540', lineHeight: 1.8 }}>{featuredArticle.summary}</p>
                </div>
                <span style={{ fontSize: 20, flexShrink: 0, color: '#c8410a', marginTop: 4 }}>→</span>
              </div>
              <p style={{ fontSize: 12, color: '#8a8480', marginTop: 16 }}>{featuredArticle.date}</p>
            </div>
          )}
        </div>
